...
class MetricsStatistics:
    def __init__(self, save_dir="./results/"):
        ...
        # Store per-epoch sample-level metric values
        self.metric_values = defaultdict(list)

        # Store epoch-wise averaged metrics
    ...
    def cal_epoch_metric(self, metrics, label_type, label, pred, pred_prob=None):
        """
        label, pred: binary masks (torch int/bool)
        pred_prob: probability map (float in [0,1]), used only for AUC
        """
        for x in metrics:
            if x not in self.func_dct:
                continue
    ...
    def cal_hausdorff(self, pred, label):
        ...
        # If both prediction and ground truth are empty → perfect match
