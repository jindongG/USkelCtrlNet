...
from models.USkelCtrlUnet import USkelCtrlUnet
...
class TrainManager:
    def __init__(self, model_dct, dataloader_producer):
        ...
        # Initialize loss module (composite vessel segmentation loss)
        ...
        # Define full evaluation metric list
    ...
    def get_model(self, model_dct):
        name = model_dct["name"]
        if "USkelCtrlUnet" == model_dct["name"]:
            model = USkelCtrlUnet(img_ch=3, output_ch=1, layer_depth=model_dct["layer_depth"],
                ...
                device)
        elif "SwinSnake_Alter" == model_dct["name"]:
    ...
    # -----------------------------
    # Optimizer + warm-up + cosine learning rate schedule
    # -----------------------------
    def reset(self):
        if self.save_weight:
    ...
    def record_performance(self, train_loader, val_loader, test_loader, epoch, metrics_statistics):
        ...
        def record_dataloader(dataloader, loader_type):
            ...
            with torch.no_grad():
                for images, labels, sample_ids in dataloader:
                    ...
                    # Save segmentation outputs as .npy (test set only, every 5 epochs)
        ...
        # Record per-sample metrics for test set
